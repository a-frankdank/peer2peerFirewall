    ...
    with pydivert.WinDivert("tcp or udp") as w:
        for packet in w:
            ...
            if network_line is None:
                ...
                network_line['process'] = renew_process(network_line['process'], networkPacket, process_connections)
                network_line['packet'] = networkPacket
                network_line['count'] += 1
            if to_refresh is not None:
                network_line = network_lines[to_refresh]
                network_line['process'] = renew_process(network_line['process'], network_line['packet'], process_connections)
                to_refresh = None
            # print("NEW ONE")
